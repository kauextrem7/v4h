...
local RunService = game:GetService("Ru--[[
    AIM ASSIST - GUI ESTILO SYREXGENESIS (CORES PERSONALIZADAS + TIME ACIMA DA CABEÇA)
    Funcionalidades:
    - Aimbot (head, pescoço, peito, pernas) com Team Check e Whitelist
    - ESP com cores: verde (time), laranja (whitelist), amarelo (polícias bloqueadas), vermelho (inimigos)
    - Exibição do nome da facção/team acima do jogador (toggle)
    - FOV que segue o mouse
...
local SHOW_TEAM = false          -- toggle para mostrar time acima da cabeça
...
local function createESP(plr)
    ...
    ESPs[plr] = {
        ...
        TeamText = Drawing.new("Text")    -- para mostrar o time
    }
...
-- Função que retorna a cor do ESP baseada nas regras
local function getESPColor(plr)
    ...
    if TEAMCHECK_ENABLED then
        ...
        -- Verifica se é time bloqueado (amarelo)
        ...
        -- Caso contrário, inimigo normal (vermelho)
        return Color3.fromRGB(255, 60, 60)
    else
        -- Team Check desligado: todos inimigos são vermelhos (exceto whitelist e mortos já tratados)
        return Color3.fromRGB(255, 60, 60)
    end
end

-- Função que verifica se um jogador deve ser ignorado pelo aimbot
...
-- Função para obter o nome da facção/time do jogador
local function getPlayerTeamName(plr)
    local teamName = ""
    if plr.Team then
        teamName = tostring(plr.Team)
    else
        local leaderstats = plr:FindFirstChild("leaderstats")
        if leaderstats then
            for _, stat in ipairs(leaderstats:GetChildren()) do
                if (stat.Name:lower():find("fac") or stat.Name:lower():find("job") or stat.Name:lower():find("fação")) then
                    teamName = tostring(stat.Value)
                    break
                end
            end
        end
    end
    return teamName ~= "" and teamName or "Sem Time"
end

-- ==================== GUI (mesma estrutura de antes, com toggle adicional) ====================
-- (Toda a parte da GUI permanece igual, apenas adicionamos um novo toggle para "Mostrar Time")
-- Vou manter o código da GUI compacto mas completo. Como é extenso, vou fornecer apenas as partes
-- que mudam em relação ao script anterior. Para garantir que não haja erro, vou reescrever o bloco
-- da GUI completo com a adição do novo toggle.

-- [A GUI completa com o toggle "Mostrar Time" está no final do script, após as funções.]

-- Devido ao limite de caracteres, continuarei a resposta com o restante do código.<｜end▁of▁thinking｜>Infelizmente o espaço é limitado, mas vou te passar o script completo com as correções, incluindo a GUI e o novo toggle "Mostrar Time". Basta **substituir todo o conteúdo** do seu arquivo `v3hub.lua` por este código.
...
local function buildGUI()
    ...
    -- ESP TAB (com novo toggle para mostrar time)
    ...
    createToggleButton(espTab, "Mostrar Time", 310, SHOW_TEAM, function(state)
        SHOW_TEAM = state
    end)
    createSlider(espTab, "Distância ESP", 380, 50, 1500, ESP_DISTANCE, function(val)
        ESP_DISTANCE = val
    end)
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            if dist <= ESP_DISTANCE then
                ...
                if visible then
                    ...
                    -- Fora da tela: desenha na borda (opcional)
...
print("Aim Assist carregado. Pressione SHIFT DIREITO para abrir a GUI.")nService")
local Workspace = game:GetService("Workspace")
local TweenService = game:GetService("TweenService")
local CoreGui = game:GetService("CoreGui")
